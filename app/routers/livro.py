...
async def listar_livros(
    ...
    livros = await livro_service.listar(db, page, limit, genero, autor, q)
    return livros
...
async def pegar_livro(
    ...
    livro = await livro_service.obter_por_idorisbn(db, isbn)
    if not livro:
        raise HTTPException(
            status_code=404,
...
async def criar_livro(
    ...
    return await livro_service.get_or_fetch(db, data.nome)